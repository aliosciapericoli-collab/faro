...
import { ArrowDownRight, ArrowUpRight, Eye, Wallet } from "lucide-react";
import {
  AreaChart,
  Area,
  XAxis,
...
import { leggiReportGoogleAds, type ReportGoogleAds } from "@/lib/analytics-google-ads.functions";
...
export function AnalyticsGoogleAds({ propertyId }: { propertyId: string }) {
    ...
    <div className="grid gap-4">
      ...
      {campaigns.length > 0 && (
        <div className="rounded-lg border border-border bg-card p-4">
          ...
          <p className="text-xs text-muted-foreground">Ultimi 28 giorni · ordinate per spesa</p>
          <ul className="mt-3 space-y-2.5">
            {campaigns.map((c) => {
              const max = campaigns[0]?.cost || 1;
              return (
